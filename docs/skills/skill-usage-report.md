...
| --- | ---: | ---: | --- |
| `root-cause-first` | 11 | 64.7% | 崩溃、配置链路、运行引擎、SAM 后端与输入事件定位 |
| `deeplux-qt-ui-audit` | 6 | 35.3% | 主窗口与子窗口主题、布局、交互状态和截图验收 |
| **合计** | **17** | **100%** | 以问题修复与界面质量保障为主 |

## 解读

根因定位的使用量约为界面审计的两倍，说明项目当前仍以稳定性、配置一致性和跨层交互修复为主要投入。界面审计已覆盖主题同步、主界面改版和真实 GUI 截图验收，但还没有形成按版本固定执行的验收门禁。

相较 2026-08-06 的快照新增 2 次：统一文件/文件夹选择器的真实 GUI 审核 1 次；文件对话框嵌套事件循环引发悬空指针崩溃的根因定位 1 次。
