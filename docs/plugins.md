...
| [逻辑控制](#逻辑控制-9) | 9 | 条件判断、延时、条件分支、循环、并行执行（实验性，未开放）、队列输入、队列输出、停止循环、条件循环 |
...
### `并行执行` - 多分支并行控制（实验性，未开放）

- **状态**：实验性。执行引擎尚未实现真正的并行控制流，该插件当前不会按并行语义执行，已从工具箱隐藏，禁止加入生产流程。
- **用途**：（规划中）声明多个并行处理分支，用于可独立执行的流程段。
- **检查**：在并行执行能力正式交付前，请勿在工程中使用该模块。
