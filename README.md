...
| 快速测量 | 无需流程即可拾取测量：两点距离、点到线距离、两线段间距、三点角度、三点拟合圆、矩形面积（拖拽）。 |
| 插件扩展 | 图像处理、检测识别、几何测量、标定、逻辑、通信、变量、3D 和相机插件按需加载。 |
| 2D/3D 数据 | 流程可处理图像与点云；3D 两点距离额外给出 ΔX/ΔY/ΔZ 分量，结果面板随主题自适应。 |
...
3. 将 `GrabImage` 的**图像源**设为“文件”，填写**图像路径**（支持单个文件或整个文件夹，文件夹模式可开启**末尾循环**）；也可切换到相机或演示输入。
...
插件位于 `src/plugins/<domain>/<PluginName>/`，通常包含 `CMakeLists.txt`、`metadata.json` 和 `*Plugin.{h,cpp}`。参数编辑器、范围和结果字段由 `metadata.json` 的 `ui.parameters` 描述；新增插件或修改接口后，应完整重建并同步插件，避免主程序与插件接口版本不一致。

插件参数遵循统一契约，由 `test_pluginparametercontracts` 持续校验：

- 默认参数必须通过 `validateParams()`；校验只约束类型、枚举、范围和字段关系，文件路径、串口等外部资源允许暂时为空，由 `process()` 运行时报告。
- `clone()` 须完整保留参数且实例相互独立；高级配置弹窗基于草稿副本编辑，确定时以单条可撤销记录提交差异。
- 每个非内部参数（不以 `_` 开头）都应有 Schema 条目；不渲染的参数需显式标记 `hidden: true`。
- 枚举默认值必须出现在 `_options` 中；数值默认值须落在 `min`/`max` 范围内。
