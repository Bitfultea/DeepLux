...
// 旋转矩形（[cx,cy,l1,l2,deg]），逐矩形取旋转角的包含式轴对齐包围盒
// （下界 floor/上界 ceil，分数边界不向内丢像素、小尺寸矩形不产生零宽）
// 与图像求交裁剪；无交集矩形记 valid=false 行（不静默丢弃），全部无交集
// 失败关闭。输出裁剪图组 crop_images（Table：矩形回显/裁剪框/
