...
// 自身 invalidValue 参数，哨兵按源图存储精度量化（键存在但类型错误或非有限
// 均失败关闭）；TiffLoader 重复极值 NoData 自动检测（原始像素单位启发式）
// 可经 autoNoData 关闭，携带契约时让位（小 ROI 也不再整幅两遍扫描），歧义图
// 失败关闭不猜测；平滑对无效位置原样保留 NaN（不补洞），缺失条带不会伪造成
// 下降/上升沿。未找到合格拐角时按契约输出 measureFailValue 且
