...
double LinesDistancePlugin::calculateSegmentDistance(double ax1, double ay1, double ax2, double ay2, double bx1,
    ...
    constexpr double kRelativeEps = 64.0 * std::numeric_limits<double>::epsilon();

    const double dax = ax2 - ax1, day = ay2 - ay1;
    const double dbx = bx2 - bx1, dby = by2 - by1;
    const double lenA = std::hypot(dax, day);
    const double lenB = std::hypot(dbx, dby);
    const double denom = dax * dby - day * dbx;
    const double denomTolerance = kRelativeEps * lenA * lenB;

    // 阶1-交叉: 两直线不平行且交点同时落在两线段内 → 距离 0，最近点=交点。
    if (qAbs(denom) > denomTolerance) {
        const double t = ((bx1 - ax1) * dby - (by1 - ay1) * dbx) / denom;
        const double s = ((bx1 - ax1) * day - (by1 - ay1) * dax) / denom;
        if (t >= -kRelativeEps && t <= 1.0 + kRelativeEps && s >= -kRelativeEps && s <= 1.0 + kRelativeEps) {
            const double boundedT = qBound(0.0, t, 1.0);
            const double ix = ax1 + boundedT * dax;
            const double iy = ay1 + boundedT * day;
            nearAx = ix;
            nearAy = iy;
            nearBx = ix;
            nearBy = iy;
    ...
    const double offsetScale = qMax(std::hypot(bx1 - ax1, by1 - ay1), std::hypot(bx2 - ax1, by2 - ay1));
    const double crossTolerance = kRelativeEps * lenA * offsetScale;
    if (lenA > 0.0 && qAbs(denom) <= denomTolerance && qAbs(crossAB) <= crossTolerance) {
        // 沿 A 方向参数化（长度量纲）
        ...
        const double projectionTolerance = kRelativeEps * lenA * qMax(lenA, offsetScale);
        if (lo <= hi + projectionTolerance) {
            // 重叠起点（沿 A 参数 lo 归一化）
