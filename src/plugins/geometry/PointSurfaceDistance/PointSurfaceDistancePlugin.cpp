...
PointSurfaceDistancePlugin::PointSurfaceDistancePlugin(QObject* parent) : ModuleBase(parent) {
...
bool PointSurfaceDistancePlugin::process(const ImageData& input, ImageData& output) {
    ...
    // 计算点到平面的距离
    m_resultDistance =
        calculatePointToPlaneDistance(point->x, point->y, point->z, plane->p1.x, plane->p1.y, plane->p1.z, plane->p2.x,
                                      plane->p2.y, plane->p2.z, plane->p3.x, plane->p3.y, plane->p3.z);
    ...
    if (norm > 1e-10) {
        ...
        // 计算垂足
        double t = (nx * (plane->p1.x - point->x) + ny * (plane->p1.y - point->y) + nz * (plane->p1.z - point->z)) /
                   (norm * norm);
