...
LoopModule::LoopModule(QObject* parent) : ModuleBase(parent) {
    m_name = "循环开始";
...
bool LoopModule::process(const ImageData& input, ImageData& output) {
    ...
    if (m_currentIndex < m_cycleCount) {
        Logger::instance().debug(QString("Loop: iteration %1 of %2").arg(m_currentIndex + 1).arg(m_cycleCount),
                                 "Logic");
        return true; // 继续循环
    }
...
LoopEndModule::LoopEndModule(QObject* parent) : ModuleBase(parent) {
    m_name = "循环结束";
