...
struct LayoutMetrics {
    ...
    int compactIconButtonSize = 30; // 紧凑图标按钮

    // 工具栏
    int toolbarHeight = 48;     // 工具栏高度
    int iconSize = 20;          // 图标尺寸

    // 间距 / 边距
    int baseSpacing = 8;        // 基础间距
    int contentMargin = 12;     // 内容边距

    // 面板装饰
    int panelRadius = 4;        // 面板圆角
    int borderWidth = 1;       // 边框宽度
    int splitterHandleWidth = 5; // Splitter 拖动区宽度

    // 面板宽度
    int toolPanelWidth = 210;   // 工具面板宽
    int flowPanelWidth = 300;   // 流程面板宽

    // 检查器
    int inspectorWidth = 280;   // 检查器宽
    int inspectorMinWidth = 240; // 检查器最小宽
    int inspectorMaxWidth = 360; // 检查器最大宽
    int collapsedHeaderHeight = 32; // 折叠栏高度

    // 底部面板
    int bottomPanelHeight = 200;  // 底部面板默认高
    int bottomPanelMinHeight = 120; // 底部面板最小高
    int bottomPanelMaxHeight = 320; // 底部面板最大高

    /**
     * @brief 返回当前主题下用于 QSplitter handle 的宽度。
     */
    int splitterHandleWidthForTheme() const { return splitterHandleWidth; }
};
