...
public:
    ...
    QVector3D center() const {
        return QVector3D(m_origin.x() + m_size * 0.5f, m_origin.y() + m_size * 0.5f, m_origin.z() + m_size * 0.5f);
    }

    // 节点访问
    std::shared_ptr<OctreeNode> root() const {
        return m_root;
    }
...
private:
    ...
    void buildNode(std::shared_ptr<OctreeNode>& node, const OctreeNodeInfo& info, const PointCloudData& pointCloud,
                   const std::vector<size_t>& pointIndices);

    QVector3D computeAverageColor(const PointCloudData& pointCloud, const std::vector<size_t>& indices) const;
