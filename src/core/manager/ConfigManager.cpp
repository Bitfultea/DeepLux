...
ConfigManager::ConfigManager() : m_configPath(PathUtils::configPath() + "/config.json") {
    setDefaults();
}
...
bool ConfigManager::save() {
    ...
    if (!PathUtils::ensureDirExists(configInfo.absolutePath()))
        return false;
...
QVariant ConfigManager::groupValue(const QString& group, const QString& key, const QVariant& defaultValue) const {
    ...
    if (defaultGroup.contains(key))
        return defaultGroup.value(key).toVariant();
