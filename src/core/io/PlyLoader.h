...
private:
    static bool parseHeader(QTextStream& stream, int& vertexCount, QList<Property>& props, bool& isBinary,
                            QString& errorMsg);
    static bool parseAscii(QTextStream& stream, int vertexCount, const QList<Property>& props, PointCloudData& out);
    static bool parseBinary(QFile& file, qint64 dataStart, int vertexCount, const QList<Property>& props,
                            PointCloudData& out);
};
